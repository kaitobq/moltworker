...
For OpenAI authenticated gateway (BYOK) with legacy settings:
...
`BRAVE_API_KEY` is passed to the gateway process via environment variable only. It is not written to persisted config JSON (`openclaw.json` / `clawdbot.json`).

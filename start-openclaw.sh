...
if [ "$RESTORE_FROM_R2" = "true" ] && [ -f "$BACKUP_DIR/openclaw/openclaw.json" ]; then
    echo "Restoring from R2 backup at $BACKUP_DIR/openclaw..."
    cp -a "$BACKUP_DIR/openclaw/." "$CONFIG_DIR/"
    cp -f "$BACKUP_DIR/.last-sync" "$CONFIG_DIR/.last-sync" 2>/dev/null || true
    echo "Restored config from R2 backup"
elif [ "$RESTORE_FROM_R2" = "true" ] && [ -f "$BACKUP_DIR/clawdbot/clawdbot.json" ]; then
...
elif [ "$RESTORE_FROM_R2" = "true" ] && [ -f "$BACKUP_DIR/clawdbot.json" ]; then
...
if [ -d "$BACKUP_DIR/workspace" ] && [ "$(ls -A "$BACKUP_DIR/workspace" 2>/dev/null)" ]; then
    WORKSPACE_SOURCE="$BACKUP_DIR/workspace"
elif [ -d "$BACKUP_DIR/clawd" ] && [ "$(ls -A "$BACKUP_DIR/clawd" 2>/dev/null)" ]; then
    WORKSPACE_SOURCE="$BACKUP_DIR/clawd"
fi
if [ "$RESTORE_FROM_R2" = "true" ] && [ -n "$WORKSPACE_SOURCE" ]; then
    echo "Restoring workspace from $WORKSPACE_SOURCE..."
    mkdir -p "$WORKSPACE_DIR"
    cp -a "$WORKSPACE_SOURCE/." "$WORKSPACE_DIR/"
    echo "Restored workspace from R2 backup"
fi

# Restore skills from R2 backup if available (only if R2 is newer)
SKILLS_DIR="/root/clawd/skills"
if [ "$RESTORE_FROM_R2" = "true" ] && [ -d "$BACKUP_DIR/skills" ] && [ "$(ls -A "$BACKUP_DIR/skills" 2>/dev/null)" ]; then
